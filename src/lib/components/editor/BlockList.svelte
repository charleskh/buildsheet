...
<section class="blocks">
  {#each build.blocks as block, i (block.id)}
    <article class="block-card" id="block-{block.id}">
      <header>
        <button
          type="button"
          class="collapse"
  ...
  {#if build.blocks.length === 0}
    <p class="empty">No sections yet. Add one below to start documenting the build.</p>
  {/if}
...
<style>
  ...
  .block-card { border: 1px solid var(--border, #444); background: var(--surface, #141414); }
  .block-card header {
    display: flex; justify-content: space-between; align-items: center;
    padding: 0.5rem 0.75rem; border-bottom: 1px solid var(--border-muted, #333);
  }
  .collapse { background: none; border: 0; color: inherit; font: inherit; font-weight: 600; cursor: pointer; }
  .controls { display: flex; gap: 0.25rem; }
  .body { padding: 0.75rem; display: flex; flex-direction: column; gap: 0.75rem; }
  .empty { color: var(--text-muted, #999); }
  .add-menu { display: grid; grid-template-columns: repeat(auto-fill, minmax(14rem, 1fr)); gap: 0.5rem; }
  .add-menu button { text-align: left; padding: 0.6rem 0.75rem; display: flex; flex-direction: column; gap: 0.15rem; }
  .add-menu span { font-size: 0.8rem; color: var(--text-muted, #999); }
  .add-menu .cancel { grid-column: 1 / -1; text-align: center; }
</style>
